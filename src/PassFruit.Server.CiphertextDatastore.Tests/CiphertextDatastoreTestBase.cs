...
namespace PassFruit.Server.CiphertextDatastore.Tests {
    ...
    public abstract class CiphertextDatastoreTestBase {
                                                             ...
                                                             CipheredAccountDto cipheredAccountDto)
        {
            ...
            ciphertextDatastore.GetAllIds().Should().Contain(id => id.Equals(accountId));
            ciphertextDatastore.GetAll().Should().Contain(datastoreCipheredAccountDto => accountId == datastoreCipheredAccountDto.Id);
            ciphertextDatastore.Get(accountId).Equals(cipheredAccountDto).Should().BeTrue();
            ciphertextDatastore.Get(accountId).Deleted.Should().Be(cipheredAccountDto.Deleted);
            ciphertextDatastore.Get(accountId).Id.Should().Be(cipheredAccountDto.Id);
            ciphertextDatastore.Get(accountId).Ciphertext.Should().Equal(cipheredAccountDto.Ciphertext);
            ciphertextDatastore.Get(accountId).InitializationVector.Should().Equal(cipheredAccountDto.InitializationVector);
            ciphertextDatastore.Get(accountId).Salt.Should().Equal(cipheredAccountDto.Salt);
        }
        ...
        private void WhenAddingASingleAccountToADataStore_ItShouldBeAdded(CiphertextDatastoreBase ciphertextDatastore, int expectedCount)
        {
            ...
            // Then
            ciphertextDatastore.GetAllIds().Should().HaveCount(expectedCount);
            ciphertextDatastore.GetAll(CipheredAccountStatus.Any).Should().HaveCount(expectedCount);
            ciphertextDatastore.GetAll(CipheredAccountStatus.Active).Should().HaveCount(expectedCount);
            ciphertextDatastore.GetAll(CipheredAccountStatus.Deleted).Should().HaveCount(0);
